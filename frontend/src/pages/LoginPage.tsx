...
export function LoginPage() {
  ...
  const [email, setEmail] = useState('Psychologist@gmail.com');
        ...
        >
          ...
          <div className="pt-4 border-t border-line">
            <div className="text-[11px] uppercase tracking-wider text-muted font-semibold mb-2">
              {t('auth.demo', { pw: 'password123' })}
            </div>
            <div className="grid grid-cols-1 gap-1.5 text-xs">
              <DemoAccount email="Admin@SocialMind.org" label={t('auth.role_admin')} onPick={setEmail} />
              <DemoAccount email="Psychologist@gmail.com" label={t('auth.role_psychologist')} onPick={setEmail} />
              <DemoAccount email="Parent@gmail.com" label={t('auth.role_parent')} onPick={setEmail} />
            </div>
          </div>
        </form>
      </div>
...
function DemoAccount({ email, label, onPick }: { email: string; label: string; onPick: (e: string) => void }) {
  return (
    <button
      type="button"
      onClick={() => onPick(email)}
      className="flex items-center justify-between gap-3 px-3 py-1.5 rounded-md border border-line hover:border-accent/50 hover:bg-white/[0.03] transition text-start"
    >
      <code className="text-slate-200 font-mono truncate">{email}</code>
      <span className="text-[10px] uppercase tracking-wider text-muted shrink-0">{label}</span>
    </button>
  );
}
